...
class AggVFL:
    ...
    def Base_train(self, X, y, num_classes, representation_features, name):
        ...
        self.task_extractor, self.task_classifier, _, self.task_optimizer = data_method_select(self.data_type, num_classes, X[0].shape[1], representation_features)  # 选择模型
        self.task_extractor, self.task_classifier = self.task_extractor.to(self.device), self.task_classifier.to(self.device)
        ...
        for epoch in range(self.N):
            for batch_X_task, batch_X_data, batch_y_data in train_dataloader:  # 遍历任务方X，数据方X，y
                ...
                outputs_task = self.task_classifier(representation_task)  # 任务方分类
                outputs_data = self.data_classifier(representation_data)  # 数据方分类
                outputs = torch.add(self.weight_task * outputs_task, self.weight_data * outputs_data)  # 合成分类
            ...
            with torch.no_grad():
                ...
                for batch_X_task_test, batch_X_data_test, batch_y_data_test in test_dataloader:
                    ...
                    representation_data_test = self.data_extractor(batch_X_data_test.unsqueeze(1).permute(0, 2, 1).float())
                    outputs_task_test = self.task_classifier(representation_task_test)
                    outputs_data_test = self.data_classifier(representation_data_test)
                    outputs_test = torch.add(self.weight_task * outputs_task_test, self.weight_data * outputs_data_test)  # 合成分类
        ...
        # # 保存模型参数
        # torch.save(self.task_extractor.state_dict(), os.path.join(self.dir, 'task_extractor.pkl'))
        # torch.save(self.task_classifier.state_dict(), os.path.join(self.dir, 'task_classifier.pkl'))
